...
(function(undefined) {
    ...
    function Schedule(opt) {
        var def = {},
            ...
            excellant = opt.excellant ? filterDate(opt.excellant) : [], //优秀
            fine = opt.fine ? filterDate(opt.fine) : [], //良好
            mild = opt.mild ? filterDate(opt.mild) : [], //轻度污染
            medium = opt.medium ? filterDate(opt.medium) : [], //中度污染
            severity = opt.severity ? filterDate(opt.severity) : [], //重度污染、
            serious = opt.serious ? filterDate(opt.serious) : [], //严重污染、
        ...
        var render = function() {
            ...
            for (var i = 0; i < total; i++) {
                ...
                var isexcellant = excellant.indexOf(nowDate) > -1;
                var isFine = fine.indexOf(nowDate) > -1;
                var isMild = mild.indexOf(nowDate) > -1;
                var isMedium = medium.indexOf(nowDate) > -1;
                var isSeverity = severity.indexOf(nowDate) > -1;
                var isSerious = serious.indexOf(nowDate) > -1;
                ...
                if (i < startWeek) {
                    ...
                    if (isDisbale) {
                        ...
                        isexcellant && (addClass = 'excellant');
                        isFine && (addClass = 'fine');
                        isMild && (addClass = 'mild');
                        isMedium && (addClass = 'medium');
                        isSerious && (addClass = 'serious');
                        isSeverity && (addClass = 'severity');
